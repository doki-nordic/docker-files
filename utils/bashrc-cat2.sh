...
_prompr_get_message () {
	...
	if [[ "$_path" != "" ]]; then
		...
		echo -ne "$(realpath "$_path/..")/\e[1;32m$(basename $(realpath "$_path"))\e[1;34m/$(realpath -s --relative-to="$_path" "$PWD")\e[0m"
	else
		echo -ne "\033]0;$(basename $PWD)\007"
		echo -ne "\e[0m$PWD"
	fi
	echo -e "\e[38;5;243m ${NRFUTIL_TOOLCHAIN_MANAGER_PROMPT_PREFIX}\e[0m"
}
